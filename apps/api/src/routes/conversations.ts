...
export async function conversationsRoutes(app: FastifyInstance) {
  ...
  app.get('/conversations', async (request: AuthReq, reply) => {
  ...
  app.get('/conversations/:id', async (request: AuthReq, reply) => {
    const { id } = request.params as { id: string };
    const convo = await convos.getConversationWithMessages(id);
  ...
  app.post('/conversations/:id/messages', async (request: AuthReq, reply) => {
    const { id } = request.params as { id: string };
    ...
    const message = await convos.addMessage(id, {
      ...body.data,
      direction: 'OUTBOUND',
  ...
  app.patch('/conversations/:id/read', async (request: AuthReq, reply) => {
    const { id } = request.params as { id: string };
  ...
  app.patch('/conversations/:id/resolve', async (request: AuthReq, reply) => {
    const { id } = request.params as { id: string };
  ...
  app.patch('/conversations/:id/ai', async (request: AuthReq, reply) => {
    const { id } = request.params as { id: string };
...
const AddMessageSchema = z.object({
  ...
  channel: z.enum(['SMS', 'EMAIL', 'VOICE', 'CHAT', 'INSTAGRAM', 'FACEBOOK']),
});
