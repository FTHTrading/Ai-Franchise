...
export async function workflowsRoutes(app: FastifyInstance) {
  ...
  app.get('/workflows', async (request, reply) => {
    ...
    const workflows = await wfService.listForOrganization(organizationId);
  ...
  app.post('/workflows/install', async (request, reply) => {
    const { organizationId } = request.authContext;
    if (!organizationId) return reply.code(403).send({ error: 'No org context' });
    ...
    const workflow = await wfService.installTemplate({
      organizationId,
      clientAccountId: body.data.clientAccountId,
      templateKey: body.data.templateKey as never,
    });
    return reply.code(201).send({ workflow });
  });

  // PATCH /workflows/:id/activate
  app.patch('/workflows/:id/activate', async (request, reply) => {
    const { id } = request.params as { id: string };
    if (!hasPermission(request.authContext.role, 'workflows:publish')) {
      return reply.code(403).send({ error: 'Forbidden' });
    }
    const wf = await wfService.activate(id, request.authContext.organizationId!);
    return reply.send({ workflow: wf });
  });

  // PATCH /workflows/:id/pause
  app.patch('/workflows/:id/pause', async (request, reply) => {
    const { id } = request.params as { id: string };
    const wf = await wfService.pause(id, request.authContext.organizationId!);
    return reply.send({ workflow: wf });
  });
